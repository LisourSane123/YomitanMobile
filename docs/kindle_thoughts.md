...
- **Karty na poziomie telefonu:** te same zip-y co w katalogu apki (kanjium pitch, KANJIDIC), pola budowane przez `AnkiCardCreator.rebuildFields` (losowa czcionka, wykres akcentu, rozbiór kanji), lokalizacja `pl` w Robolectricu, żeby etykiety były po polsku.
  - Ustawienia stylu pochodzą z `settings.json` backupu apki (`~/.local/share/kindle-sync/settings.json`). Kopia odświeża się przez adb za każdym razem, gdy telefon jest podłączony — a że przy synchronizacji z Kindle zwykle nie jest, `kindle-sync.sh --pull-settings` robi sam ten krok (raz, z podłączonym telefonem, po zrobieniu kopii zapasowej w apce) i mówi, na czym stanął, jeśli się nie uda. Bez niej czcionki są odczytywane z ostatnich 300 kart telefonu. To jedyne ustawienie stylu, które trafia do pól karty, bo reszta siedzi w CSS note type'a, a ten synchronizuje telefon.
...
- **AutoReorder po dodaniu:** wtyczka sama uruchamia się tylko przy starcie Anki i z menu, a AnkiConnect nie może jej wywołać. Jej algorytm (10 linijek) jest więc powtórzony w `KindleSync.AnkiConnect.reorder`, z ustawieniami czytanymi z konfiguracji wtyczki (`meta.json` nadpisuje `config.json`). Pozycje zapisuje `setSpecificValueOfCard` (`update_card`), więc zmiana synchronizuje się jak każda inna. Sprawdzone: na talii Japanese wynik zgadza się z tym, co zostawiła wtyczka (0 przesunięć). Wtyczka sortuje tylko `deck:Japanese is:new`, więc talia docelowa musi leżeć w środku tego wyszukiwania. `KindleSync` sprawdza to przed przestawianiem (`reorder_covers` w `summary.txt`): jeśli nie obejmuje, nie przestawia nic i mówi o tym w powiadomieniu — przy `shift_existing` przestawienie zepchnęłoby świeże karty za całą istniejącą kolejkę.
