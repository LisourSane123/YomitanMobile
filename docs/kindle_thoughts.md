...
- **Karty na poziomie telefonu:** te same zip-y co w katalogu apki (kanjium pitch, KANJIDIC), pola budowane przez `AnkiCardCreator.rebuildFields` (losowa czcionka, wykres akcentu, rozbiór kanji), lokalizacja `pl` w Robolectricu, żeby etykiety były po polsku.
  ...
  - Audio: **VOICEVOX** (neuronowy, offline, `voicevox_core` 0.17 w venv, modele w `~/.local/share/kindle-sync/voicevox/core`). Open JTalk zostaje tylko jako zapas, bo jego głos HMM brzmiał robotycznie przy neuronowym TTS telefonu. Głośność była w porządku, zła była jakość. VOICEVOX dostaje **czytanie z akcentem wpisanym z kanjium** (notacja AquesTalk: `ツキツケ'ル`), więc nagranie mówi dokładnie to, co karta pokazuje na wykresie akcentu. Głos jest losowany per słowo spośród neutralnych lektorów (VOICEVOX Nemo 女声1-3/男声1-2, No.7 アナウンス), a nie spośród głosów postaci. Nagrania są normalizowane do −16 LUFS, a nazwa pliku zawiera silnik (`yomitan_kindle_vv_…`), bo AnkiDroid cache'uje media po nazwie.
