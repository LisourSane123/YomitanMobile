...
- **Filtering** (`domain/usecase/TextScanPlanner`): same contract as `JlptDeckPlanner` — fixed rule order, first rejection owns the counter, `selected + skipped == distinct words`. Adds a `FUNCTION_WORDS` stoplist (particles, です/ます, する/いる…): they are genuine dictionary entries at the top of every frequency list, so without it the first hundred cards are pure grammar. `knownTokenCount` counts only words skipped as already-in-Anki / already-mined / function words, so the "you already know X% of this text" figure is not inflated by words dropped for being rare.
- **Shared rules** (`domain/usecase/WordFilterRules`): the archaic and proper-name tag rules used to live inside `JlptDeckPlanner`; both planners answer the same question about the same data, so they now share one implementation.
